...
public class CheckoutSagaOrchestrator {
    ...
    public OrderDto executeSaga(CheckoutSagaContext context) {
        ...
        try {
            ...
            for (int i = 0; i < 60; i++) {
                Optional<Order> opt = sagaStateService.findOrderById(winningSaga.getOrderId());
                if (opt.isPresent()) {
                    Order o = opt.get();
                    if (o.getStatus() == OrderStatus.CONFIRMED || o.getStatus() == OrderStatus.FAILED || o.getStatus() == OrderStatus.PROCESSING) {
                        winningOrder = o;
                        break;
                    }
                }
            ...
            if (winningOrder == null) {
                winningOrder = sagaStateService.findOrderById(winningSaga.getOrderId())
    ...
    private void publishOrderEvent(Order order, String eventType, String routingKey) {
        try {
            ...
            CorrelationData correlationData = new CorrelationData("saga-order-" + order.getId() + "-" + eventType);
