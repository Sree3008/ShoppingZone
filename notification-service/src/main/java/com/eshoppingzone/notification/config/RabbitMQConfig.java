...
public class RabbitMQConfig {
    ...
    public RabbitTemplate rabbitTemplate(ConnectionFactory connectionFactory) {
        ...
        rabbitTemplate.setConfirmCallback((correlationData, ack, cause) -> {
            ...
            if (ack) {
                log.debug("[notification-service] Message confirmed by broker. correlationId={}", id);
            } else {
                log.warn("[notification-service] Message NACK'd by broker. correlationId={}, cause={}", id, cause);
            }
        });

        rabbitTemplate.setReturnsCallback(returned -> {
            log.error("[notification-service] Message returned (unroutable). exchange={}, routingKey={}, replyCode={}, replyText={}",
