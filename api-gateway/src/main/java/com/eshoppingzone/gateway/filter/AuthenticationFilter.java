...
public class AuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // Correlation ID: reuse existing X-Correlation-ID or generate new UUID
        String incomingCorrelationId = request.getHeaders().getFirst("X-Correlation-ID");
        final String correlationId = (incomingCorrelationId != null && !incomingCorrelationId.trim().isEmpty())
                ? incomingCorrelationId.trim()
                : java.util.UUID.randomUUID().toString();

        exchange.getResponse().getHeaders().set("X-Correlation-ID", correlationId);

        // Sanitize incoming request by stripping client-supplied identity headers to prevent header injection/spoofing
        ...
        if (sanitizedRequest.getMethod().name().equalsIgnoreCase("GET") && path.startsWith("/api/v1/products")) {
            isPublic = true;
        }
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (jwtUtil.isTokenValid(token)) {
                ...
                ServerHttpRequest.Builder builder = sanitizedRequest.mutate();
                if (userId != null) {
