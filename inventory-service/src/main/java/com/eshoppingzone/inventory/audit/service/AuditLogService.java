package com.eshoppingzone.inventory.audit.service;

import com.eshoppingzone.inventory.audit.context.AuditContext;
import com.eshoppingzone.inventory.audit.dto.AuditEvent;
import com.eshoppingzone.inventory.audit.dto.AuditLogDto;
...
import com.eshoppingzone.inventory.audit.repository.AuditLogSpecification;
import com.eshoppingzone.inventory.security.UserPrincipal;
...
public class AuditLogService {
    ...
    public AuditLog log(String action,
        ...
        try {
            ...
            if (actorUserId == null || actorUsername == null || actorRole == null) {
                Authentication auth = SecurityContextHolder.getContext().getAuthentication();
                if (auth != null && auth.isAuthenticated() && !"anonymousUser".equals(auth.getPrincipal())) {
                    if (auth.getPrincipal() instanceof UserPrincipal principal) {
                        if (actorUserId == null) actorUserId = principal.getUserId();
                        if (actorUsername == null) actorUsername = principal.getUsername();
                        if (actorRole == null) actorRole = principal.getRole();
                    } else {
                        if (actorUsername == null) actorUsername = auth.getName();
