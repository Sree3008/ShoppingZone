...
class InventoryServiceTest {
    ...
    @Test
    void testAddStock() {
        when(inventoryRepository.findByProductIdWithLock(100L)).thenReturn(Optional.of(sampleInventory));
    ...
    void testRestockSuccess() {
        ...
        when(stockMovementRepository.existsByReferenceAndMovementType("RET-12345", MovementType.ADD_STOCK)).thenReturn(false);
        when(inventoryRepository.findByProductIdWithLock(100L)).thenReturn(Optional.of(sampleInventory));

        inventoryService.restock(request);

        assertEquals(55, sampleInventory.getAvailableStock());
        verify(inventoryRepository, times(1)).save(sampleInventory);
        verify(stockMovementRepository, times(1)).save(any(StockMovement.class));
    }

    @Test
    void testRestockDuplicateSkipped() {
        StockReservationRequest request = new StockReservationRequest(
                "RET-12345",
                List.of(new StockReservationItem(100L, 5))
        );

        when(stockMovementRepository.existsByReferenceAndMovementType("RET-12345", MovementType.ADD_STOCK)).thenReturn(true);
