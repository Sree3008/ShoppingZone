...
public class RefundController {
    ...
    public ResponseEntity<ApiResponse<RefundDto>> requestRefund(
        ...
        RefundDto refund = paymentService.requestRefund(customerId, request, effectiveKey);
        return ResponseEntity.ok(ApiResponse.success("Refund requested successfully", refund));
    }

    @PostMapping("/internal")
    @Operation(summary = "Internal Request Refund", description = "Internal endpoint for order-service to initiate return refund")
    @PreAuthorize("hasAnyRole('INTERNAL', 'ADMIN')")
    public ResponseEntity<ApiResponse<RefundDto>> requestRefundInternal(
            @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
            @Valid @RequestBody RefundRequest request) {
        String effectiveKey = (idempotencyKey != null && !idempotencyKey.isBlank())
                ? idempotencyKey : (request != null ? request.getIdempotencyKey() : null);
        RefundDto refund = paymentService.requestRefundInternal(request, effectiveKey);
        return ResponseEntity.ok(ApiResponse.success("Refund requested successfully", refund));
    }
